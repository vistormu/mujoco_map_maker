...
class MapMaker:
    ...
    def _add_asset(self, body: Body) -> None:
        if body.type == 'mesh' and body.mesh_file:
            mesh_attr: str = XMLParser.create_attribute('name', body.name)
            file_attr: str = XMLParser.create_attribute('file', body.mesh_file)

            asset_element: str = XMLParser.create_element('asset', [mesh_attr, file_attr], None)

            self.assets.append(asset_element)
    ...
    def _add_body(self, body: Body) -> None:
        ...
        color_attr: str = XMLParser.create_attribute('rgba', (*body.color, body.alpha) if type(body.color) is tuple else _hex_to_rgba(body.color, body.alpha))
